...
<body>
    ...
    <section id="con-servicios">
        <div class="container">
			...
			<?
			$cadenaSQL = "SELECT camp0, tipo, titulo, foto, descripcion, orden, url2 FROM nosotros WHERE tipo = 'c2' order by orden DESC ";
			//$cadenaSQL = $cadenaSQL.$cadenaSQL1;
			$result = mysql_query($cadenaSQL, $dbh);
			while($row = mysql_fetch_array($result))
			{
				...
				<div class="grid-servicios">
					...
					<div id="cont-detalla">
						<div class="mini-container2">
							...
							<p><? echo substr($row["descripcion"],0,180); ?></p>
							<!--a href="<? echo $row["url2"]; ?>">Ver Más</a-->
							<a href="servicios1b.php?id=<? echo $row["camp0"]; ?>">Ver Más</a>
						</div>
					</div>
				</div>
